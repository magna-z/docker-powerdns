...
LABEL pro.magnaz.docker.powerdns.version="{\"container\": 1.4, \"alpine\": 3.4, \"powerdns\": 4.0.1}"
...
RUN export POWERDNS_VERSION=4.0.1 && \
    ...
    ./configure --prefix="" --exec-prefix=/usr --sysconfdir=/etc/powerdns --with-modules="gmysql remote" && \
    make && make install && \
